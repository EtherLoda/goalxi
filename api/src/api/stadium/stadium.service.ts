...
import { StadiumEntity, STADIUM_COST_PER_SEAT, STADIUM_DEMOLISH_REFUND_RATE } from '@goalxi/database';
...
export class StadiumService {
     ...
     * 建造新球场（替换现有）
     */
    async build(teamId: string, dto: BuildStadiumReqDto): Promise<{ stadium: StadiumEntity; cost: number }> {
        if (dto.capacity < 1000) {
            throw new BadRequestException('Capacity must be at least 1000');
        }

        // 检查是否已有球场
        const existing = await this.stadiumRepository.findOne({ where: { teamId } });
        let refund = 0;

        if (existing) {
            // 如果已有球场，计算拆除返还
            if (existing.isBuilt) {
                refund = Math.floor(existing.capacity * STADIUM_COST_PER_SEAT * STADIUM_DEMOLISH_REFUND_RATE);
            }
            // 删除旧球场
        ...
        this.logger.log(`Stadium built for team ${teamId}: capacity=${dto.capacity}, cost=${cost}`);
     ...
     * 调整球场容量（增减座位）
     */
    async resize(teamId: string, dto: ResizeStadiumReqDto): Promise<{ stadium: StadiumEntity; cost: number }> {
        if (dto.capacity < 1000) {
            throw new BadRequestException('Capacity must be at least 1000');
        }
        ...
        if (capacityDiff > 0) {
            ...
            // 减少座位：返还部分费用
            cost = Math.floor(capacityDiff * STADIUM_COST_PER_SEAT * STADIUM_DEMOLISH_REFUND_RATE);
        }

        stadium.capacity = dto.capacity;
        await this.stadiumRepository.save(stadium);

        this.logger.log(`Stadium resized for team ${teamId}: ${oldCapacity} -> ${dto.capacity}, cost=${cost}`);
    ...
    async demolish(teamId: string): Promise<{ refund: number }> {
        ...
        // 计算返还金额
        const refund = Math.floor(stadium.capacity * STADIUM_COST_PER_SEAT * STADIUM_DEMOLISH_REFUND_RATE);
