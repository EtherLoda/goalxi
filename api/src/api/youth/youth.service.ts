...
import { YouthPlayerEntity, PlayerEntity, TeamEntity, PlayerAbility } from '@goalxi/database';
...
export class YouthService {
    ...
    async findByTeam(teamId: string): Promise<YouthPlayerEntity[]> {
        return this.youthRepo.find({ where: { teamId, isPromoted: false }, order: { joinedAt: 'DESC' } });
    }
