...
describe('YouthService', () => {
        ...
        it('should not exceed potential values', async () => {
            const youth = createYouthPlayer({
                currentSkills: {
                    ...
                    technical: { finishing: 14.9, passing: 14.9, dribbling: 14.9, defending: 14.9 },
            ...
            const saved = youthRepo.save.mock.calls[0][0] as YouthPlayerEntity;
            expect((saved.currentSkills as any).physical.pace).toBeLessThanOrEqual(15);
        });
    });
    ...
    describe('promote', () => {
        it('should create senior player and mark youth as promoted', async () => {
            const youth = createYouthPlayer({
                revealedSkills: ['pace', 'strength', 'finishing', 'passing', 'dribbling', 'defending', 'positioning', 'composure', 'freeKicks', 'penalties'],
            });
        ...
        it('should throw if player already promoted', async () => {
            ...
            await expect(service.promote('youth-1')).rejects.toThrow('already promoted');
        });
    });
