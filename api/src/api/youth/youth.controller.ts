import { Controller, Get, Post, Param, BadRequestException, UseGuards } from '@nestjs/common';
import { AuthGuard } from '../../guards/auth.guard';
import { CurrentUser } from '../../decorators/current-user.decorator';
import { YouthService } from './youth.service';
import { YouthPlayerEntity, TeamEntity } from '@goalxi/database';
...
export class YouthController {
    ...
    async promote(
        ...
        if (youth.revealedSkills.length < keys.length * 0.5) {
            throw new BadRequestException('Player not yet fully scouted (need at least 50% skills revealed)');
        }
