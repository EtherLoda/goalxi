...
import { StaffEntity, StaffRole, StaffLevel, Uuid } from '@goalxi/database';
import { TeamEntity, FinanceEntity } from '@goalxi/database';
...
export class StaffsService {
    ...
    async findByTeam(teamId: string): Promise<StaffEntity[]> {
        return this.staffRepo.find({ where: { teamId, isActive: true }, order: { role: 'ASC' } });
    }
    ...
    async hire(teamId: string, role: StaffRole, level: StaffLevel, userId: string): Promise<StaffEntity> {
        // Check if role already filled
        const existing = await this.staffRepo.findOne({ where: { teamId, role, isActive: true } });
        if (existing) {
        ...
        const finance = await this.financeRepo.findOne({ where: { teamId: teamId as Uuid } });
    ...
    async fire(staffId: string, userId: string): Promise<void> {
        ...
        if (!staff.isActive) throw new BadRequestException('Staff already inactive');
        ...
        const remainingWeeks = Math.max(1, Math.ceil(remainingMs / (7 * 24 * 60 * 60 * 1000)));
        const terminationFee = remainingWeeks * staff.salary * 2;

        const finance = await this.financeRepo.findOne({ where: { teamId: staff.teamId as Uuid } });
    ...
    /** Toggle auto-renewal */
    async setAutoRenew(staffId: string, autoRenew: boolean): Promise<StaffEntity> {
        const staff = await this.findOne(staffId);
    ...
    getTrainingQualityScore(teamId: string, staffList: StaffEntity[]): number {
        const head = staffList.find(s => s.role === StaffRole.HEAD_COACH && s.isActive);
        const fitness = staffList.find(s => s.role === StaffRole.FITNESS_COACH && s.isActive);
        const psych = staffList.find(s => s.role === StaffRole.PSYCHOLOGY_COACH && s.isActive);
        const tech = staffList.find(s => s.role === StaffRole.TECHNICAL_COACH && s.isActive);
        ...
        return Math.round(headScore * 0.6 + fitnessScore * 0.15 + psychScore * 0.15 + techScore * 0.10);
    }

    private generateStaffName(role: StaffRole, level: StaffLevel): string {
        const firstNames = ['John', 'Mike', 'Carlos', 'Hans', 'Pierre', 'Marco', 'Yuki', 'Ali', 'David', 'Luis'];
        const lastNames = ['Smith', 'Garcia', 'Mueller', 'Dubois', 'Rossi', 'Tanaka', 'Santos', 'Kim', 'Jensen', 'Obi'];
        const prefix = {
