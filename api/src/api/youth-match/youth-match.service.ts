...
import {
  YouthMatchListResDto,
  YouthMatchResDto,
} from './dto/youth-match.res.dto';
...
export class YouthMatchService {
  ...
  async findAll(
    filters: ListYouthMatchesReqDto,
  ...
  async getTactics(
    ...
    homeTactics: YouthTacticsResDto | null;
    awayTactics: YouthTacticsResDto | null;
  }> {
    const match = await this.matchRepository.findOne({
      where: { id: matchId },
    });
  ...
  async submitTactics(
    ...
    if (
      match.homeYouthTeamId !== youthTeamId &&
      match.awayYouthTeamId !== youthTeamId
    ) {
      throw new ForbiddenException(
        'Youth team is not participating in this match',
      );
    }
    ...
    if (!youthTeam) {
      throw new NotFoundException(
        `Youth team with ID ${youthTeamId} not found`,
      );
    }
  ...
  async validateYouthTeamOwnership(
    userId: string,
    youthTeamId: string,
  ): Promise<boolean> {
    const youthTeam = await this.youthTeamRepository.findOne({
  ...
  private mapTacticsToResDto(
    tactics: YouthMatchTacticsEntity,
  ): YouthTacticsResDto {
    return {
      id: tactics.id,
