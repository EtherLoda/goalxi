...
import { FanEntity, FAN_HIDDEN_CAP, FAN_BASE_GROWTH, FAN_BASE_LOSS, FAN_CAP_SMOOTHING } from '@goalxi/database';
...
export class FanService {
    ...
    async weeklyUpdate(
        ...
        await this.fanRepository.save(fan);
        this.logger.debug(`Fan weekly update for team ${teamId}: ${change >= 0 ? '+' : ''}${change}, total: ${fan.totalFans}`);
    ...
    calculateAttendance(
        ...
        // 总入场不超过容量
        return Math.min(capacity, neutralFans + homeFansAttendance + awayFansAttendance);
    }
}
