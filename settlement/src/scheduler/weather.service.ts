...
export class WeatherService {
     ...
     * 获取指定日期和地点的天气
     */
    async getWeatherByDate(date: string, locationId: string = this.DEFAULT_LOCATION): Promise<WeatherEntity | null> {
     ...
     * 获取今天的实际天气
     */
    async getTodayWeather(locationId: string = this.DEFAULT_LOCATION): Promise<WeatherEntity | null> {
     ...
     * 获取明天的天气预报（2-3个选项）
     */
    async getTomorrowForecast(locationId: string = this.DEFAULT_LOCATION): Promise<WeatherForecast[]> {
    ...
    generateForecastsFromToday(todayWeather: WeatherType): WeatherForecast[] {
        ...
        const totalWeight = selectedArray.reduce((sum, w) => sum + transition[w], 0);

        const mainProbability = 40 + Math.floor(Math.random() * 31);
        let remainingProbability = 100 - mainProbability;

        const mainWeather = selectedArray[Math.floor(Math.random() * selectedArray.length)];
        forecasts.push({
            weather: mainWeather,
        ...
        for (let i = 0; i < remaining.length; i++) {
            ...
            if (i === remaining.length - 1) {
                ...
                const maxAssign = Math.min(remainingProbability - (remaining.length - i - 1) * 10, 40);
                const minAssign = Math.max(10, remainingProbability - 50);
                prob = minAssign + Math.floor(Math.random() * (maxAssign - minAssign + 1));
            }
     ...
     * 创建或更新今日天气记录
     */
    async createOrUpdateTodayWeather(locationId: string = this.DEFAULT_LOCATION): Promise<WeatherEntity> {
        const today = this.formatDate(new Date());

        const yesterday = await this.getWeatherByDate(this.formatDate(this.addDays(new Date(), -1)), locationId);
