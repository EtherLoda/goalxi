...
export class MatchSchedulerService {
     ...
     */
    private async getTeamTactics(matchId: string, teamId: string): Promise<MatchTacticsEntity | null> {
        // 1. 查找比赛提交的战术
        ...
        if (defaultPreset) {
            this.logger.debug(`[MatchPreprocessScheduler] Using default preset for team ${teamId}`);
     ...
     * 将战术预设转换为比赛战术实体
     */
    private presetToMatchTactics(preset: TacticsPresetEntity, matchId: string, teamId: string): MatchTacticsEntity {
