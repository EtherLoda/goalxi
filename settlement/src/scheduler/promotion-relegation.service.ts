...
export class PromotionRelegationService {
    ...
    private async getLowerLeague(league: LeagueEntity): Promise<LeagueEntity | null> {
        return this.leagueRepository.findOne({
            where: { tier: league.tier + 1, tierDivision: league.tierDivision },
        });
    }

    private async updateTeamLeagueAndStanding(teamId: string, newLeagueId: string, season: number): Promise<void> {
