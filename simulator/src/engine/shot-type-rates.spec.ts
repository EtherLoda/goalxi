...
describe('Shot Type Goal Rates', () => {
  ...
  const runMatches = (
    ...
    let totalShotsAll = 0;
    let totalMoments = 0;
    const laneDistribution = { left: 0, center: 0, right: 0 };
    const laneShots = { left: 0, center: 0, right: 0 };
    const laneGoals = { left: 0, center: 0, right: 0 };
        ...
        for (const event of events) {
          if (
            ...
            if (shot && shot.shotType) {
              ...
              allShotTypes[shotTypeName] =
                (allShotTypes[shotTypeName] || 0) + 1;
            }
          }
        }
  ...
  for (const [formationName, positions] of Object.entries(FORMATIONS)) {
    it(`should show different goal rates for each shot type (${formationName}, OVR 12 vs 12)`, () => {
      ...
      const {
        ...
        laneShots,
        laneGoals,
      } = runMatches(12, 12, 500, formationName);

      let totalGoals = 0;

      for (const [shotType, data] of Object.entries(shotStats)) {
        const rate =
          data.total > 0 ? ((data.goals / data.total) * 100).toFixed(1) : 'N/A';
        console.log(
          `${shotType}: ${data.goals}/${data.total} goals (${rate}%)`,
        );
      ...
      const pushSuccessRate =
        totalMoments > 0
          ? ((totalShots / totalMoments) * 100).toFixed(1)
          : 'N/A';
      const shotConversionRate =
        totalShots > 0 ? ((totalGoals / totalShots) * 100).toFixed(1) : 'N/A';
      const overallRate =
        totalShots > 0 ? ((totalGoals / totalShots) * 100).toFixed(1) : 'N/A';
      console.log(
      ...
      console.log(
        `Push success rate: ${pushSuccessRate}% (${totalShots}/${totalMoments})`,
      );
      console.log(`Shot conversion rate: ${shotConversionRate}%`);

      // Lane distribution
      const totalLaneEvents =
        laneDistribution.left +
        laneDistribution.center +
        laneDistribution.right;
      console.log(`\nLane distribution (shot events):`);
      if (totalLaneEvents > 0) {
        console.log(
          `  Left:   ${((laneDistribution.left / totalLaneEvents) * 100).toFixed(1)}% | Shots: ${laneShots.left} | Goals: ${laneGoals.left} | Conv: ${laneShots.left > 0 ? ((laneGoals.left / laneShots.left) * 100).toFixed(1) : 'N/A'}%`,
        );
        console.log(
          `  Center: ${((laneDistribution.center / totalLaneEvents) * 100).toFixed(1)}% | Shots: ${laneShots.center} | Goals: ${laneGoals.center} | Conv: ${laneShots.center > 0 ? ((laneGoals.center / laneShots.center) * 100).toFixed(1) : 'N/A'}%`,
        );
        console.log(
          `  Right:  ${((laneDistribution.right / totalLaneEvents) * 100).toFixed(1)}% | Shots: ${laneShots.right} | Goals: ${laneGoals.right} | Conv: ${laneShots.right > 0 ? ((laneGoals.right / laneShots.right) * 100).toFixed(1) : 'N/A'}%`,
        );
      } else {
  ...
  it('detailed analysis for 4-2-3-1', () => {
    const { laneData, totalMoments } = runMatchesDetailed(
      12,
      12,
      500,
      '4-2-3-1',
    ...
    console.log(
      'Lane      | Attacks | Shots | Push%   | Goals | Conv%  | ONE_ON_ONE | HEADER | REBOUND | NORMAL | LONG_SHOT',
    );
    console.log(
      '----------|---------|-------|---------|-------|--------|------------|--------|---------|--------|----------',
    );
    ...
    for (const lane of ['left', 'center', 'right']) {
      ...
      const pushRate =
        d.attacks > 0 ? ((d.shots / d.attacks) * 100).toFixed(1) : '0.0';
      const convRate =
        d.shots > 0 ? ((d.goals / d.shots) * 100).toFixed(1) : '0.0';
      const st = (name: string) => {
        const s = d.shotTypes[name];
        return s.total > 0 ? `${((s.goals / s.total) * 100).toFixed(0)}%` : '-';
      };
      console.log(
        `${lane.padEnd(10)}| ${String(d.attacks).padStart(7)} | ${String(d.shots).padStart(5)} | ${pushRate.padStart(7)}% | ${String(d.goals).padStart(5)} | ${convRate.padStart(6)}% | ${st('ONE_ON_ONE').padStart(10)} | ${st('HEADER').padStart(6)} | ${st('REBOUND').padStart(7)} | ${st('NORMAL').padStart(6)} | ${st('LONG_SHOT').padStart(9)}`,
      );
    }

    const totalAttacks =
      laneData.left.attacks + laneData.center.attacks + laneData.right.attacks;
    const totalShots =
      laneData.left.shots + laneData.center.shots + laneData.right.shots;
    const totalGoals =
      laneData.left.goals + laneData.center.goals + laneData.right.goals;
    const overallPush =
      totalAttacks > 0 ? ((totalShots / totalMoments) * 100).toFixed(1) : '0.0';
    const overallConv =
      totalShots > 0 ? ((totalGoals / totalShots) * 100).toFixed(1) : '0.0';

    console.log(
      '----------|---------|-------|---------|-------|--------|------------|--------|---------|--------|----------',
    );
    console.log(
      `${'TOTAL'.padEnd(10)}| ${String(totalAttacks).padStart(7)} | ${String(totalShots).padStart(5)} | ${overallPush.padStart(7)}% | ${String(totalGoals).padStart(5)} | ${overallConv.padStart(6)}%`,
    );
  ...
  it('4312 vs 532 per-lane analysis (500 matches)', () => {
    ...
    const printTeamStats = (
      ...
      for (const lane of ['left', 'center', 'right']) {
        const s = stats[lane];
        const ratio =
          s.defPower > 0 ? (s.attPower / s.defPower).toFixed(2) : '-';
        const pushRate =
          s.attacks > 0
            ? ((s.pushSuccess / s.attacks) * 100).toFixed(1)
            : '0.0';

        console.log(
          `${lane.padEnd(8)}| ${String(s.possession).padStart(10)} | ${ratio.padStart(5)} | ${pushRate.padStart(7)}% | ${s.goals}`,
        );
      }
    };
    ...
    const printLaneComparison = () => {
      ...
      console.log(
        'Lane    | Home Poss | Away Poss | Total | Home%  | Away%  | Ratio',
      );
      console.log(
        '--------|-----------|-----------|-------|--------|--------|------',
      );
      ...
      for (const lane of ['left', 'center', 'right']) {
        ...
        const homePct =
          total > 0 ? ((homePoss / total) * 100).toFixed(1) : '0.0';
        const awayPct =
          total > 0 ? ((awayPoss / total) * 100).toFixed(1) : '0.0';
        const homeRatio = awayPoss > 0 ? (homePoss / awayPoss).toFixed(2) : '-';
        console.log(
          `${lane.padEnd(8)}| ${String(homePoss).padStart(11)} | ${String(awayPoss).padStart(9)} | ${String(total).padStart(5)} | ${homePct.padStart(6)}% | ${awayPct.padStart(6)}% | ${homeRatio}`,
        );
      }
    };
  ...
  it('4231-v2 vs 442-diamond per-lane analysis (500 matches)', () => {
      ...
      for (const event of events) {
        ...
        if (
          ...
          if (lane && laneStats[lane]) {
            ...
            if (push?.defensePower)
              laneStats[lane].defPower += push.defensePower;
            if (push?.success) laneStats[lane].pushSuccess++;

            // 推进方式统计
            if (attackType) {
              if (!attackStats[attackType])
            ...
            if (shot) {
              ...
              const shotType = shot.shotType as string;
              if (!shotStats[shotType])
                shotStats[shotType] = { total: 0, goals: 0 };
    ...
    console.log(
      `场均: ${avgHomeGoals} vs ${avgAwayGoals} | 胜率: ${homeWinRate}% vs ${awayWinRate}% | 平率: ${drawRate}%`,
    );
    ...
    for (const lane of ['left', 'center', 'right']) {
      ...
      const homeRatio =
        home.defPower > 0 ? (home.attPower / home.defPower).toFixed(2) : '-';
      const awayRatio =
        away.defPower > 0 ? (away.attPower / away.defPower).toFixed(2) : '-';
      const homePush =
        home.attacks > 0
          ? ((home.pushSuccess / home.attacks) * 100).toFixed(1)
          : '0.0';
      const awayPush =
        away.attacks > 0
          ? ((away.pushSuccess / away.attacks) * 100).toFixed(1)
          : '0.0';
      console.log(
        `${lane.padEnd(8)}| ${homeRatio.padStart(10)} | ${homePush.padStart(10)}% | ${awayRatio.padStart(10)} | ${awayPush.padStart(10)}%`,
      );
    }
    ...
    console.log('--------|------|------|------|--------|------');
    const atTotal = Object.values(homeAttackTypeStats).reduce(
      (sum, s) => sum + s.total,
      0,
    );
    for (const [type, stats] of Object.entries(homeAttackTypeStats).sort(
      (a, b) => b[1].total - a[1].total,
    )) {
      const pct =
        atTotal > 0 ? ((stats.total / atTotal) * 100).toFixed(1) : '0.0';
      const successRate =
        stats.total > 0
          ? ((stats.success / stats.total) * 100).toFixed(1)
          : '0.0';
      console.log(
        `${type.padEnd(8)}| ${String(stats.total).padStart(4)} | ${pct.padStart(6)}% | ${String(stats.success).padStart(4)} | ${successRate.padStart(7)}% | ${stats.shots}`,
      );
    }
    ...
    console.log('推进方式 | 次数 | 占比 | 成功 | 成功率 | 射门');
    console.log('--------|------|------|------|--------|------');
    const atTotal2 = Object.values(awayAttackTypeStats).reduce(
      (sum, s) => sum + s.total,
      0,
    );
    for (const [type, stats] of Object.entries(awayAttackTypeStats).sort(
      (a, b) => b[1].total - a[1].total,
    )) {
      const pct =
        atTotal2 > 0 ? ((stats.total / atTotal2) * 100).toFixed(1) : '0.0';
      const successRate =
        stats.total > 0
          ? ((stats.success / stats.total) * 100).toFixed(1)
          : '0.0';
    ...
    console.log('--------|------|------|------|--------');
    const stTotal = Object.values(homeShotTypeStats).reduce(
      (sum, s) => sum + s.total,
      0,
    );
    for (const [type, stats] of Object.entries(homeShotTypeStats).sort(
      (a, b) => b[1].total - a[1].total,
    )) {
      const pct =
        stTotal > 0 ? ((stats.total / stTotal) * 100).toFixed(1) : '0.0';
      const goalRate =
        stats.total > 0
          ? ((stats.goals / stats.total) * 100).toFixed(1)
          : '0.0';
      console.log(
        `${type.padEnd(8)}| ${String(stats.total).padStart(4)} | ${pct.padStart(6)}% | ${String(stats.goals).padStart(4)} | ${goalRate.padStart(7)}%`,
      );
    }
    ...
    console.log('射门方式 | 次数 | 占比 | 进球 | 成功率');
    console.log('--------|------|------|------|--------');
    const stTotal2 = Object.values(awayShotTypeStats).reduce(
      (sum, s) => sum + s.total,
      0,
    );
    for (const [type, stats] of Object.entries(awayShotTypeStats).sort(
      (a, b) => b[1].total - a[1].total,
    )) {
      const pct =
        stTotal2 > 0 ? ((stats.total / stTotal2) * 100).toFixed(1) : '0.0';
      const goalRate =
        stats.total > 0
          ? ((stats.goals / stats.total) * 100).toFixed(1)
          : '0.0';
  ...
  it('4312 vs 532 matchup (500 matches)', () => {
    ...
    console.log(
      `Home (4-3-1-2): ${result.homeGoals} goals, ${result.homeShots} shots`,
    );
    console.log(
      `Away (5-3-2): ${result.awayGoals} goals, ${result.awayShots} shots`,
    );
    console.log(
      `Total: ${totalGoals} goals, avg ${(totalGoals / 500).toFixed(2)} per match`,
    );
    console.log('\nShot types:');
