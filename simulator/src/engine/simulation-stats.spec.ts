...
describe('Simulation Statistics', () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      for (const goal of goalEvents) {
        ...
        stats.attackTypeCounts[attackType] =
          (stats.attackTypeCounts[attackType] || 0) + 1;

        // 统计射门类型
        stats.shotTypeCounts[shotType] =
          (stats.shotTypeCounts[shotType] || 0) + 1;
      }
    }
  ...
  describe('Equal Teams (OVR 12)', () => {
    it('should have balanced results with equal teams', () => {
      ...
      console.log(
        `Home Goals: ${stats.homeGoals} (${((stats.homeGoals / totalGoals) * 100).toFixed(1)}%)`,
      );
      console.log(
        `Away Goals: ${stats.awayGoals} (${((stats.awayGoals / totalGoals) * 100).toFixed(1)}%)`,
      );
      ...
      for (const [type, count] of Object.entries(stats.goalsByType)) {
        console.log(
          `  ${type}: ${count} (${((count / totalGoals) * 100).toFixed(1)}%)`,
        );
      }
      console.log(`\nAttack Type Distribution:`);
      for (const [type, count] of Object.entries(stats.attackTypeCounts)) {
        const totalAttacks = Object.values(stats.attackTypeCounts).reduce(
          (a, b) => a + b,
          0,
        );
        console.log(
          `  ${type}: ${count} (${((count / totalAttacks) * 100).toFixed(1)}%)`,
        );
      }
      ...
      expect(avgGoalsPerMatch).toBeLessThan(8.0); // 合理范围：0.3-8球/场
      expect(goalDiff / totalGoals).toBeLessThan(0.2); // 差距小于20%
    });
  });
  ...
  describe('Unequal Teams (OVR 8 vs OVR 16)', () => {
    it('should show advantage for stronger team', () => {
      ...
      console.log(`Avg Goals/Match: ${avgGoalsPerMatch.toFixed(2)}`);
      console.log(
        `Strong/Weak Ratio: ${(strongTeamGoals / Math.max(1, weakTeamGoals)).toFixed(2)}`,
      );
  ...
  describe('Detailed Stats Comparison', () => {
    it('should provide detailed comparison', () => {
      ...
      console.log(
        `Goal Diff: ${Math.abs(equalStats.homeGoals - equalStats.awayGoals)}`,
      );
      ...
      console.log(
        `Ratio: ${(unequalStats.awayGoals / Math.max(1, unequalStats.homeGoals)).toFixed(2)}x`,
      );
      ...
      console.log('\n========== Summary ==========');
      console.log(
        `OVR 12 vs OVR 12: ${(equalStats.homeGoals + equalStats.awayGoals).toFixed(0)} total goals, ${((equalStats.homeGoals + equalStats.awayGoals) / 1000).toFixed(2)} per match`,
      );
      console.log(
        `OVR 8 vs OVR 16: ${(unequalStats.homeGoals + unequalStats.awayGoals).toFixed(0)} total goals, ${((unequalStats.homeGoals + unequalStats.awayGoals) / 1000).toFixed(2)} per match`,
      );
