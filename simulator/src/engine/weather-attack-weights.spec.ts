...
describe('Weather Attack Weights', () => {
  ...
  describe('specific weather effects', () => {
    ...
    it('windy should boost CROSS and LONG_SHOT (wind advantage)', () => {
      ...
      expect(weights[0]).toBe(1.2); // CROSS boosted
    ...
    it('foggy should heavily reduce THROUGH_PASS', () => {
      ...
      expect(weights[2]).toBe(0.6); // THROUGH_PASS most reduced
    });
  ...
  describe('weighted distribution calculation', () => {
    ...
    it('windy should increase CROSS proportion compared to cloudy', () => {
      ...
      // Windy weights
      const windyWeighted = distribution.map(
        (base, i) => base * WEATHER_ATTACK_WEIGHTS['windy'][i],
      );
      ...
      const cloudyWeighted = distribution.map(
        (base, i) => base * WEATHER_ATTACK_WEIGHTS['cloudy'][i],
      );
    ...
    it('foggy should significantly reduce THROUGH_PASS proportion', () => {
      ...
      const foggyWeighted = distribution.map(
        (base, i) => base * WEATHER_ATTACK_WEIGHTS['foggy'][i],
      );
  ...
  describe('edge cases', () => {
    it('should handle unknown weather by using cloudy defaults', () => {
      ...
      const weights =
        WEATHER_ATTACK_WEIGHTS[unknownWeather] ||
        WEATHER_ATTACK_WEIGHTS['cloudy'];
