...
export class YouthSimulationProcessor extends WorkerHost {
  ...
  private async runSimulation(match: YouthMatchEntity): Promise<void> {
    ...
    const [homeTeamEntity, awayTeamEntity] = await Promise.all([
      this.youthTeamRepository.findOne({
        where: { id: match.homeYouthTeamId as any },
      }),
      this.youthTeamRepository.findOne({
        where: { id: match.awayYouthTeamId as any },
      }),
    ]);
        ...
        .values(
          events.map((e) => {
            ...
            return {
              ...
              typeName: e.type,
              teamId:
                e.teamName === match.homeYouthTeam.name
                  ? match.homeYouthTeamId
                  : e.teamName === match.awayYouthTeam.name
                    ? match.awayYouthTeamId
                    : null,
